...
import StockHealthPanel from './dashboard/StockHealthPanel';
...
type Zone3Tab = 'menu' | 'stock' | 'pnl' | 'revenue';
...
export default function OwnerDashboard({ onClose }: Props) {
  ...
  const [stockData, setStockData] = useState<StockHealthData | null>(null);
  ...
  const fetchZone12 = useCallback(async () => {
    try {
      const [p, q] = await Promise.all([getPulseMetrics(), getActionQueue()]);
      setPulse(p);
  ...
  return (
    <div className="owner-dash">
      ...
      <div className="owner-dash-header">
        ...
        <button type="button" className="owner-dash-close" onClick={onClose}>
          Back to Chat
        </button>
      </div>
      ...
      <div className="owner-dash-zone1">
        {zone12Loading && !pulse ? (
          <div className="owner-dash-zone1-loading">Loading…</div>
        ) : zone12Error && !pulse ? (
          <div className="owner-dash-zone1-error">{zone12Error}</div>
        ) : pulse ? (
          <PulseStrip data={pulse} lastUpdated={pulseUpdated} />
        ) : null}
      </div>
      ...
      <div className="owner-dash-zone3">
        <div className="z3-tabs">
          {(['menu', 'stock', 'pnl', 'revenue'] as Zone3Tab[]).map((tab) => (
            <button
              key={tab}
        ...
        <div className="z3-content">
          ...
          {activeTab === 'stock' && (
            stockData
              ? <StockHealthPanel data={stockData} onRefresh={handleRefreshTab} loading={zone3Loading} />
              : <PanelSkeleton loading={zone3Loading} />
          )}
          {activeTab === 'pnl' && (
            pnlData
